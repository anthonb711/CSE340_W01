  ...
  if(data.length > 0){
    ...
    data.forEach(cartItem => { 
      ...
      cartList += '<p class="cart-card__colorMiles">'
        + 'Color: '
        + cartItem.inv_color.charAt(0).toUpperCase() + cartItem.inv_color.slice(1)+ ' '
        +' Miles: '
      ...
      cartList += '<p class="cart-card__price">'
        + '$' + new Intl.NumberFormat ('en-US').format(cartItem.total_price) + '</p>'
      cartList += '<button id="removeFromCart" class="cart-card__removeBtn" title="Remove item from cart" item-id='
        + cartItem.inv_id + '>'
    ...
    cartList += '<p class="notice">Your cart is Empty.</p>'
  }
  return cartList;
}
